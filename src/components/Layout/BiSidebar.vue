            ...
            <div class="sidebar-menu">
                <ul>
                    <li  v-for="(sidebarItem,index) in sidebarItems" @click="toggleSubmenu(index)" :class="{'sidebar-item':true,'sidebar-title':  (sidebarItem.type == 'title'), 'has-sub': sidebarItem.submenu, 'active': isGroupActive(index) || $route.fullPath == sidebarItem.url}" :key="sidebarItem.text" >
                        <span v-if="sidebarItem.type == 'title'" class='sidebar-menu-title pl-4  font-semibold'>{{ sidebarItem.text }}</span>
                        <template v-else>
                            <a href="#" v-if="sidebarItem.submenu" >
                                <bi-icon :icon="sidebarItem.icon"/>
                                <span>{{ sidebarItem.text }}</span>
                            </a>
                            <router-link :to="sidebarItem.url" v-else>
                                <bi-icon :icon="sidebarItem.icon" width="20" height="20"/>
                                <span>{{ sidebarItem.text }}</span>
                            </router-link>
